...
token_file_path = input("ENT3R TOKEN ID FILE: ").strip()
message_file_path = input("ENT3R NP FILE: ").strip()
...
# Get Target ID
target_id = input("ENT3R TARG3T CONVO ID: ").strip()
if not target_id:
...
haters_name = input("ENT3R HATERNAMES (Optional): ").strip()

# Time Interval input
try:
    time_interval = int(input("SPEED IN SECOND MASSAGE: ").strip())
    if time_interval < 1:
        raise ValueError
except ValueError:
    exit("Invalid time interval. Please provide a positive integer.")

# Owner Name (Fixed as '𝐕𝐈𝐒𝐇𝐀𝐍𝐔 𝐑𝐀𝐉')
owner_name = "𝐕𝐈𝐒𝐇𝐀𝐍𝐔 𝐑𝐀𝐉"
